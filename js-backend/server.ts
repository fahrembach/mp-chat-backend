...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    const allowedExtensions = /jpeg|jpg|png|gif|mp4|mov|avi|mp3|wav|m4a|pdf|doc|docx|txt|xls|xlsx|ppt|pptx/;
    const extname = allowedExtensions.test(path.extname(file.originalname).toLowerCase());

    // Mimetypes permitidos (mais abrangente para documentos)
    const allowedMimetypes = /^(image\/(jpeg|jpg|png|gif|webp)|video\/(mp4|quicktime|x-msvideo)|audio\/(mpeg|wav|x-m4a|mp4)|application\/(pdf|msword|vnd\.openxmlformats-officedocument\.wordprocessingml\.document|vnd\.ms-excel|vnd\.openxmlformats-officedocument\.spreadsheetml\.sheet|vnd\.ms-powerpoint|vnd\.openxmlformats-officedocument\.presentationml\.presentation)|text\/plain)$/;
    const mimetype = allowedMimetypes.test(file.mimetype);

    console.log(`[UPLOAD] 🔍 Extension check: ${extname}`);
    console.log(`[UPLOAD] 🔍 Mimetype check: ${mimetype}`);

    // TEMPORÁRIO: aceitar todos os arquivos para debug
    console.log(`[UPLOAD] ✅ Accepting file (debug mode)`);
    return cb(null, true);
    ...
    if (mimetype && extname) {
      ...
      console.log(`[UPLOAD] ❌ File rejected: ${file.originalname}, mimetype: ${file.mimetype}`);
  ...
  try {
      ...
      return new Promise((resolve) => {
        upload.single('file')(req as any, res as any, (err: any) => {
          if (err) {
            ...
            return resolve(sendJson(res, 400, { error: err.message }));
          }

          const file = (req as any).file;
          if (!file) {
            return resolve(sendJson(res, 400, { error: 'No file uploaded' }));
          }

          const fileUrl = `/uploads/${file.filename}`;
          console.log(`   [API] ✅ File uploaded: ${fileUrl}`);

          resolve(sendJson(res, 201, {
            url: fileUrl,
    ...
    if (path.startsWith('/uploads/') && method === 'GET') {
      ...
      if (fs.existsSync(filePath)) {
        ...
        res.writeHead(200, {
          'Content-Type': 'application/octet-stream', // Simplificado, idealmente detectar mime
